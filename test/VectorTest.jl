...
testbandedblockoperator(M.op.ops[1].op)

using SO
M.op.ops[1].op[1:8,1:8]|>chopm
M=Multiplication(G,space(G1))
    S=view(M.op.ops[1].op,Block(1):Block(2),Block(1):Block(2))
    ret=bbzeros(S)
    T = eltype(S)
...
M[1,1][Block(1),Block(1)]

M[1,1]
convert(BandedBlockMatrix,view(parent(S).ops[1,1].op.op,KR,JR))
parent(S).ops[1,1].op.op
domainspace(parent(S).ops[1,1])|>blocklengths
KR,JR

S=view(parent(S).ops[1,1],KR,JR)
...
K
reindex(S,parentindexes(S),(Block(1),Block(1)))
reindex(rangespace(S),(parentindexes(S)[1],),(Block(1),))

parentindexes(rangespace(S))
@which rangespace(S[K,J])
@which view(S,K,J)
rangespace(S[K,J])
blockrows(parent(S),3)

KR,JR
parent(S)[blockrows(parent(S),3),blockcols(parent(S),4)]
M
...
convert(BandedBlockMatrix,S)|>chopm

[Block(1),Block(2)]|>chopm
using SO
blockbandwidths(S)
co=cache(M)
co[1:10,1:10]
resizedata!(co,:,30)
    ...
    S=view(M.op,Block(3):Block(4),Block(3):Block(4))
    AbstractMatrix(S)
ret = bbzeros(S)
    T = eltype(S)
    KR,JR = parentindexes(S)
