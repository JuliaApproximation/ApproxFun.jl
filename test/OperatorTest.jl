...
A=Conversion(Chebyshev(d),UltrasphericalSpace{2}(d))
@test norm(A\Fun(x.*f,rangespace(A))-(x.*f)) < 100eps()

@test norm((Conversion(Chebyshev(d),UltrasphericalSpace{2}(d))\(D^2*f))-diff(diff(f))) < 100eps()

@test norm(X*f-(x.*f)) < 100eps()

A=Conversion(Chebyshev(d),UltrasphericalSpace{2}(d))*X
