...
function pde_standardize_rhs(A,f::Vector)
    ...
    ds=domainspace(A)
    fx=isempty(indsBx)?Fun{typeof(spacetype(ds,2)),eltype(A)}[]:convert2fun(f[indsBx],ds[2])
    fy=isempty(indsBy)?Fun{typeof(spacetype(ds,1)),eltype(A)}[]:convert2fun(f[indsBy],ds[1])
