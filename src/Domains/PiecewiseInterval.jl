...
function PiecewiseInterval{IT<:Segment}(pcsin::AbstractVector{IT})
    pcs=collect(pcsin)
    p=∂(pop!(pcs))
    successful=true
    while successful
...
pieces{T}(d::PiecewiseInterval{T}) = Segment{T}[d[k] for k=1:numpieces(d)]

for OP in (:arclength,:complexlength)
    @eval $OP(d::PiecewiseInterval)=mapreduce($OP,+,pieces(d))
end

Base.getindex(d::PiecewiseInterval,j::Integer) = Segment(d.points[j],d.points[j+1])
