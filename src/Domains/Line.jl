...
function chebyshevintegrate(d::Line,cfs::Vector)
    @assert d.α==d.β==-1.
    # || d.α==d.β==-.5
    
#    if domain(f).α==domain(f).β==-1.
        uneumannrange_xsqd(uneumann_dirichlet_transform(coefficients(Fun([1.5,0.,.5]).*Fun(cfs),1)))
#    end
#     elseif d.α==d.β==-.5
...
for T in {Float64,Complex{Float64}}
    function chebyshevsum(d::Line,cfs::Vector)
        if d.α==d.β==-.5
            sum(SingFun(divide_singularity(Fun(cfs)),-.5,-.5))
        else
            cf = integrate(f)
            last(cf) - first(cf)
        end
