...
function coefficients(f::Vector,sp1::JacobiWeight{Chebyshev},sp2::JacobiWeight{Chebyshev})
    ...
    if c==α && d==β
        f
    elseif c>α && d>β
        coefficients(divide_singularity(f),JacobiWeight(α+1,β+1,sp1.space),sp2)
    elseif c>α
        coefficients(divide_singularity(-1,f),JacobiWeight(α+1,β,sp1.space),sp2)
    elseif d>β
        coefficients(divide_singularity(1,f),JacobiWeight(α,β+1,sp1.space),sp2)
    else
