...
function coefficients(f::Vector,sp1::JacobiWeight{Chebyshev},sp2::JacobiWeight{Chebyshev})
    ...
    elseif c>α
        ...
        coefficients(divide_singularity(-1,g).coefficients,JacobiWeight(α+1,β,sp1.space),sp2)
    elseif d>β
        g=Fun(f,sp1.space)
        @assert abs(last(g))<tol
        coefficients(divide_singularity(1,g).coefficients,JacobiWeight(α,β+1,sp1.space),sp2)
    else
