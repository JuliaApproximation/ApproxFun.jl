...
doc"""
`WeightedLaguerre(α)` is a function space generated by `xᵅ * exp(-x) * Lₙ(x)`
where `Lₙ(x)`'s are the Laguerre polynomials in the space `Laguerre(α)`.

`WeightedLaguerre()` is equivalent to `WeightedLaguerre(0)` by default.
"""
